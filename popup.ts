...
const canvas = $("#hueRing") as HTMLCanvasElement;
const ctx = canvas.getContext("2d")!;
const thumb = $("#thumb");
const rgbInput = $("#rgb") as HTMLInputElement;
const hslInput = $("#hsl") as HTMLInputElement;
const hexInput = $("#hex") as HTMLInputElement;
const presets = $("#presets");
const historyBox = $("#history");
const copied = $("#copied") as HTMLSpanElement;
const eyedropperBtn = $("#eyedropper") as HTMLButtonElement;
...
function placeThumbByHue(hue:number) {
  const cx = canvas.width/2, cy = canvas.height/2;
...
function bind() {
  ...
  const onMove = (ev:MouseEvent|TouchEvent) => {
    ...
    if (dist < (RING_R - RING_W/2) || dist > (RING_R + RING_W/2)) return;
  ...
  presets?.addEventListener("click", (e) => {
    const btn = (e.target as HTMLElement).closest(".sw") as HTMLElement | null;
    if (!btn) return;
...
function renderHistory(list:string[]) {
